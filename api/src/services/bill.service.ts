...
import { BillStatus, OrderItemStatus, type TableStatus, type Pagination } from '@/types/index.js'
...
async function generateBillShortId(): Promise<string> {
  const result = await mongoose.connection
    ...
    .findOneAndUpdate({ _id: 'bill' }, { $inc: { seq: 1 } }, { returnDocument: 'after', upsert: true })
...
export async function getBillById(id: string): Promise<BillDetail> {
  ...
  const tableDoc = (await TableModel.findById(bill.table_id).lean()) as ITable | null
  ...
  const rawOrders = (await OrderModel.find({ bill_id: bill._id }).lean()) as IOrder[]
  ...
  const payments = (await PaymentModel.find({ bill_id: bill._id }).lean()) as IPayment[]
...
export async function listBills(query: ListBillsQuery): Promise<SuccessListResponse<IBill>> {
  ...
  const [rawItems, total] = await Promise.all([
    BillModel.find(filter).sort({ created_at: -1 }).skip(skip).limit(limit).lean(),
...
export async function splitBill(input: SplitBillInput): Promise<SplitBillResult> {
  if (!mongoose.Types.ObjectId.isValid(input.id)) {
    throw problem({
      type: 'not-found',
      title: 'Not Found',
  ...
  const orders = (await OrderModel.find({ bill_id: parentBill._id }).lean()) as IOrder[]
  const orderIds = orders.map((o) => o._id)
  ...
  const itemMap = new Map(allActiveItems.map((item) => [String(item._id), item]))
...
export async function recalculateBillTotal(bill_id: string): Promise<IBill> {
  ...
  const updated = await BillModel.findByIdAndUpdate(bill_id, { total_amount: total }, { new: true })
  if (!updated) {
    throw problem({
      type: 'not-found',
      title: 'Not Found',
