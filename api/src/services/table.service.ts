...
export async function listTables(query: ListTablesQuery): Promise<SuccessListResponse<ITable>> {
...
export async function createTable(input: CreateTableInput): Promise<ITable> {
  ...
  logger.info('Table created', { tableId: String(table._id), table_name: table.table_name })
  return table
}

export async function updateTable(id: string, input: UpdateTableInput): Promise<ITable> {
  await getTableById(id)

  if (input.table_name !== undefined) {
    const dup = await TableModel.findOne({ table_name: input.table_name, _id: { $ne: id } })
    if (dup) {
      throw problem({
        type: 'conflict',
        title: 'Conflict',
  ...
  const updated = await TableModel.findByIdAndUpdate(id, input, { new: true, runValidators: true })
  if (!updated) {
    throw problem({
      type: 'not-found',
      title: 'Not Found',
...
export async function updateTableStatus(
  ...
  if (currentStatus === 'OCCUPIED' && targetStatus === 'AVAILABLE' && table.bill_ids.length > 0) {
    throw problem({
      type: 'conflict',
      title: 'Conflict',
      status: 409,
...
export async function moveTable(
  ...
  return {
    ...
    to_table: { id: String(target._id), table_name: target.table_name, status: 'OCCUPIED' },
  }
}
