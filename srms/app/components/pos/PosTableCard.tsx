...
export default function PosTableCard({ table }: PosTableCardProps) {
  ...
  const { mutate: openTable, isPending: isOpening } = useUpdateTableStatus();
  const isActive =
    ...
    <>
      <div className="rounded-lg overflow-hidden shadow-md">
        ...
        <div className="px-5 py-4 bg-white h-[222px]">
          {displayItems.length === 0 ? (
            <div className="h-full flex items-center justify-center">
              <p className="text-center font-bold text-xl text-zinc-500">
                ບໍ່ມີລາຍການອາຫານ
              </p>
            </div>
          ) : (
            <ul className="[&_>_li]:border-b [&_>_li:last-child]:border-none [&_>_li]:pb-2 space-y-2">
              {displayItems.slice(0, 3).map((item: OrderItem) => (
                <li key={item._id}>
                  ...
                  {item.note && (
                    <p className="text-amber-600 text-sm">{item.note}</p>
                  )}
                </li>
              ))}
        ...
        <div className="px-4 py-4">
          <Button
            className="px-4 py-3 text-lg font-bold w-full bg-teal-500 hover:bg-teal-600"
            disabled={isOpening}
            onClick={
              isActive
                ? () => setDetailOpen(true)
